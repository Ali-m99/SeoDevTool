...
def handle_form(request):
    if request.method == "GET":
        domain = request.GET.get('domain-search', '').strip()
        query = request.GET.get('keyword-search', '').strip()

        # Validate that both fields are filled
        if not domain or not query:
            return render(request, 'home.html', {
                'error_message': 'Please fill in both the domain and keyword fields.',
                'domain': domain,  # Return the existing values to preserve form state
                'query': query
            })
        ...
        try:
            ...
            return render(request, 'home.html', {
                'error_message': f'Error accessing the website: {str(e)}',
                'domain': domain,
                'query': query
            })
